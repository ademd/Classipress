...
class CP_Order extends APP_View {
    ...
    function template_vars() {
        ...
        if ( APP_Gateway_registry::is_gateway_enabled( 'bank-transfer' ) ) {
			$template_vars['bt_message'] = ! empty( $cp_options->gateways['bank-transfer']['message'] ) ? $cp_options->gateways['bank-transfer']['message'] : '';
        }
...
class CP_Listing_Form_Edit extends CP_Listing_Checkout_Step {
	...
	public function update_listing( $order, $checkout ) {
		...
		$address_comps = array_intersect_key( $this->posted_fields, array(
			'cp_street'  => '',
			'cp_city'    => '',
			'cp_state'   => '',
			'cp_zipcode' => '',
		...
		cp_maybe_geocode_address( $listing->ID, $address_comps );
	...
	public function update_attachments( $order, $checkout ) {
		...
		$attach_ids = get_posts( array( 'post_parent' => $listing->ID, 'post_type' => 'attachment', 'post_mime_type' => 'image', 'nopaging' => true, 'fields' => 'ids', 'order' => 'ASC', 'orderby' => 'menu_order' ) );

		if ( count( $attach_ids ) ) {

			if ( empty( $_POST['_thumbnail_id'] ) ) {
				delete_post_meta( $listing->ID, '_thumbnail_id' );
			} elseif ( in_array( $_POST['_thumbnail_id'], $attach_ids ) ) {
				update_post_meta( $listing->ID, '_thumbnail_id', absint( $_POST['_thumbnail_id'] ) );
			}

			if ( empty( $_POST['_cp_banner_image'] ) ) {
				delete_post_meta( $listing->ID, '_cp_banner_image' );
			} elseif ( in_array( $_POST['_cp_banner_image'], $attach_ids ) ) {
				update_post_meta( $listing->ID, '_cp_banner_image', absint( $_POST['_cp_banner_image'] ) );
			}
...
class CP_Gateway_Process extends CP_Order_Checkout_Step {
	 ...
	 * @param APP_Dynamic_Checkout $checkout A checkout object.
	 *
	 * @return void
	 */
	public function process( $order, $checkout ) {
		...
		$cancel_step = 'gateway-select';

		// Prevent the redirect loop on Select Gateway step.
		if ( 1 === count( APP_Gateway_Registry::get_active_gateways() ) ) {
			$cancel_step = $checkout->get_previous_step( $cancel_step );
		}

		// Update order complete and cancel urls.
		update_post_meta( $order->get_id(), 'complete_url', appthemes_get_step_url( 'order-summary' ) );
		update_post_meta( $order->get_id(), 'cancel_url', appthemes_get_step_url( $cancel_step ) );
