...
jQuery( document ).ready( function( $ ) {
	...
	$( "#form-categorydiv .checkall" ).on( "change", function() {
		var checkedStatus = this.checked;
		$( "#categorychecklist input:checkbox" ).prop( "checked", checkedStatus );
		$( this ).closest( "label" ).find( ".checkall-label" ).html( checkedStatus ? classipress_admin_params.text_uncheck_all : classipress_admin_params.text_check_all );
		return false;
	} );
} );
...
window.themeImageWidget = window.themeImageWidget || {},
	function( a, b, c ) {
		themeImageWidget.MediaManager = function( a ) {
			this.options = a, this.media = this, this.target = a.target, this.$target = b( "#" + this.options.target ),
				this.$trigger = b( "." + this.options.target + "-add" ), this.setFrame(), this.bindEvents();
			...
			b( document ).on( "widget-added widget-updated", function( c, d ) {
				...
				a.target = a.options.target.replace( "__i__", e ), a.$target = b( "#" + a.target ),
					a.$trigger = b( "." + a.target + "-add" ), a.$trigger.on( "click", function( b ) {
...
jQuery( function( $ ) {
	$( document ).on( "widget-added", function( c, d ) {
		d.find( 'input[type="colorpicker"]' ).wpColorPicker( {
			change: function( e, ui ) {
				$( e.target ).val( ui.color.toString() );
				$( e.target ).trigger( "change" );
			}
		} ).closest( ".wp-picker-container" ).css( "display", "block" );
	} );
} );
