...
jQuery( function( $ ) {
	...
	if ( $( '.app-attachment-list' ).length > 0 ) {
		$( '.app-attachment-list' ).sortable( {
			...
			handle: ".thumbnail",
			scroll: false, // Set to false since it scrolls page to the right to mobile menu area.
	...
	$( '.refine-categories-list-label' ).on( 'click', function() {
		$( this )
			.toggleClass( 'open' )
			.next( '.refine-categories-list-wrap' )
			.animate( {
				height: [ 'toggle', 'swing' ],
				opacity: 'toggle'
			}, 200 );
		...
		$.ajax( {
			...
			success: function( data ) {
				...
				if ( data.success === true ) {
					$( '#' + results_div ).html( data.html ).slideDown( "fast" ); //build html from ajax post
