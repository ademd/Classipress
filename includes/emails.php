...
function cp_notify_ad_owner_email( $new_status, $old_status, $post ) {
	...
	} elseif ( $old_status == 'publish' && $new_status === CP_POST_STATUS_EXPIRED && $current_user->ID != $post->post_author && $send_expired_email ) {

		// Don't send emails for ads expired more than 3 days ago. Prevents mass
		// email sending after upgrade to 4.2.0 version.
		// See https://github.com/AppThemes/ClassiPress/issues/592 for more info.
		if ( strtotime( '-3 days' ) > strtotime( $post->cp_sys_expire_date ) ) {
			return;
		}
...
function cp_contact_ad_owner_email( $post_id ) {
	...
	$post = get_post( $post_id );
	$author_email = get_the_author_meta( 'user_email', $post->post_author );
	$from_name = appthemes_filter( appthemes_clean( $_POST['name'] ) );
	$from_email = appthemes_clean( $_POST['email'] );
	$subject = wp_specialchars_decode( sprintf( __( 'New inquiry on "%s"', APP_TD ), $post->post_title ), ENT_QUOTES );
