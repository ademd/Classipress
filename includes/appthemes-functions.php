...
function appthemes_get_profile_pic( $author_id, $author_email, $avatar_size ) {
	echo cp_get_avatar( $author_email, $avatar_size );
}
 ...
 *
 * @global scbOptions $cp_options The options object
 *
 * @param type $string     The price string to be cleaned up.
 * @param type $returnType Whether to return a float type or leave as the
 *                          string. Takes 'float' value.
 *
 * @return string|float
 */
function appthemes_clean_price( $string, $returnType = false ) {
	...
	/**
	 * Filters the cleaned price value
	 *
	 * @param string|float $string The price value
	 */
