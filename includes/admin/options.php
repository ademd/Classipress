...
function cp_exclude_cats( $id = null ) {
	...
	if ( $records ) {
		foreach ( $records as $record ) {
			$form_cats = unserialize( $record->form_cats );
...
function cp_add_core_fields( $form_id ) {
	...
	// check to see if there is a default form.
	$default_forms = $wpdb->get_results( "SELECT id FROM $wpdb->cp_ad_forms WHERE form_status = 'default' LIMIT 1" );

	if ( $default_forms ) {
		foreach ( $default_forms as $default_form ) {
			$fields = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->cp_ad_meta WHERE form_id = %d", $default_form->id ) );
		}
	}
						...
						<td class="forminp">
							<label>
								<div id="form-categorydiv">
									...
									<p><label><input type="checkbox" class="checkall" /><span class="checkall-label"><?php _e( 'check all', APP_TD ); ?></span></label></p>
								</div>
								<p class="description"><?php echo $value['desc']; ?></p>
							</label>
						</td>
...
?>
			...
			?>
				...
				<div class="wrap">
					...
					if ( isset($_POST['submitted']) ) {
						...
						if ( 'default' === $data['form_status'] ) {
							// No categories for deafult form.
							$data['form_cats'] = '';
							// Check if there already is any default forms.
							$default_forms = $wpdb->get_results( "SELECT ID, form_cats FROM $wpdb->cp_ad_forms WHERE form_status = 'default'" );
							if ( $default_forms ) {
								foreach ( $default_forms as $default_form ) {
									$wpdb->update( $wpdb->cp_ad_forms, array(
										'form_status' => 'inactive',
									), array( 'id' => $default_form->ID ) );
								}
							}
						}
						...
						?>
							...
							<p style="text-align:center;padding-top:50px;font-size:22px;"><?php _e( 'Creating your form.....', APP_TD ); ?><br /><br /><i class="fa fa-cog fa-spin fa-3x fa-fw"></i></p>
			...
			?>
				...
				<div class="wrap">
					...
					if ( isset( $_POST['submitted'] ) && $_POST['submitted'] == 'yes' ) {
						...
						if ( ! isset( $_POST['post_category'] ) && 'default' !== $_POST['form_status'] ) {
							wp_die( '<p style="color:red;">' . __( 'Error: Please select at least one category.', APP_TD ) . " <a href='#' onclick='history.go(-1);return false;'>" . __( 'Go back', APP_TD ) . '</a></p>' );
						}
					...
					?>
						...
						<p style="text-align:center;padding-top:50px;font-size:22px;"><?php _e( 'Saving your changes.....', APP_TD ); ?><br /><br /><i class="fa fa-cog fa-spin fa-3x fa-fw"></i></p>
						<meta http-equiv="refresh" content="0; URL=<?php echo $admin_fields_url; ?>">
		...
		*/
		...
		case 'delete':
			...
			?>
			<p style="text-align:center;padding-top:50px;font-size:22px;"><?php _e( 'Deleting form layout.....', APP_TD ); ?><br /><br /><i class="fa fa-cog fa-spin fa-3x fa-fw"></i></p>
			<meta http-equiv="refresh" content="0; URL=<?php echo $admin_fields_url; ?>">
		<?php
		break;
							...
							<tr class="<?php echo $rowclass; ?>">
								...
								<td class="column-categories">
									...
									if ( 'default' !== $result->form_status ) {
										...
										$not_empty = cp_exclude_cats();
										$empty_terms = get_terms( APP_TAX_CAT, array( 'exclude' => $not_empty, 'hide_empty' => false ) );
										$empty_terms = wp_list_pluck( $empty_terms, 'term_id' );
										echo cp_match_cats( $empty_terms );
									}
									?>
								</td>
...
?>
	...
	switch ( $theswitch ) {
				...
				if ( isset( $_POST['submitted'] ) ) {
					...
					?>
						<p style="text-align:center;padding-top:50px;font-size:22px;"><?php _e( 'Creating your field.....', APP_TD ); ?>
							<br/><br/><i class="fa fa-cog fa-spin fa-3x fa-fw"></i>
						</p>
						<meta http-equiv="refresh" content="0; URL=<?php echo $admin_fields_url; ?>" >
					<?php
					endif;
		...
		?>
			<p style="text-align:center;padding-top:50px;font-size:22px;"><?php _e( 'Deleting custom field.....', APP_TD ); ?><br /><br /><i class="fa fa-cog fa-spin fa-3x fa-fw"></i></p>
			<meta http-equiv="refresh" content="0; URL=<?php echo $admin_fields_url; ?>">
			<?php
			break;
